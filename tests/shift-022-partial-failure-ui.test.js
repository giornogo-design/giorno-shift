...
test('補償済みを自動復旧として表示する',()=>assert(html.includes("status==='COMPENSATED'")&&html.includes('自動復旧して操作前の状態へ戻しました')));
test('重大エラーへ操作IDと再操作禁止を表示する',()=>assert(html.includes("操作ID: '+(operationId")&&html.includes('同じ操作を繰り返さないでください')));
test('ok falseとneedsReviewを成功扱いしない',()=>assert(html.includes('data.ok!==true')&&html.includes("data.operationStatus==='NEEDS_REVIEW'")));
